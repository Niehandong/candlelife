...
async def login(session: AsyncSession, username: str, password: str,
    ...
    if len(password.encode("utf-8")) > MAX_PASSWORD_BYTES:
        ...
        raise HTTPException(status.HTTP_422_UNPROCESSABLE_CONTENT, "PASSWORD_TOO_LONG")
...
async def change_password(session: AsyncSession, admin: AdminUser,
    ...
    if len(new_password) < MIN_PASSWORD_LEN:
        raise HTTPException(status.HTTP_422_UNPROCESSABLE_CONTENT, "PASSWORD_TOO_SHORT")

    if verify_password(new_password, admin.hashed_password):
        raise HTTPException(status.HTTP_422_UNPROCESSABLE_CONTENT, "PASSWORD_UNCHANGED")
