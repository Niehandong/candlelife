...
from tests.conftest import err, failed
    ...
    for bad in ("gratitudes", "plans", "openid", "nickname", "access_token"):
        r = await auth_client.post("/api/v1/events", json={"events": [
            {"type": "t", "payload": {bad: "不该出现"},
             "occurred_at": "2026-08-27T23:59:00+08:00"}]})
        assert err(r) == codes.UNPROCESSABLE, bad
        assert err(r) == codes.UNPROCESSABLE
...
async def test_batch_size_capped(auth_client):
    ...
    failed(await auth_client.post("/api/v1/events", json={"events": events}),
           codes.UNPROCESSABLE)


async def test_empty_batch_rejected(auth_client):
    failed(await auth_client.post("/api/v1/events", json={"events": []}),
           codes.UNPROCESSABLE)
